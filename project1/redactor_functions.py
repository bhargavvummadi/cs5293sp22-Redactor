...
def redact_phone(file_content,write_stats_file):
    ...
    nlp_obj = spacy.load("en_core_web_sm")
    pattern1 = [ {"ORTH": "+", "OP": "?"},{"SHAPE": {"REGEX":"[1-9]{1}"}, "OP":"?"},{"ORTH": "(", "OP": "?"}, {"SHAPE": "ddd"},{"ORTH": ")", "OP": "?"}, {"SHAPE": "ddd"}, {"ORTH": "-", "OP": "?"},
               {"SHAPE": "dddd"}]
        ...
        if t == '\n':
            ...
            if flag == 1:
                for match_id, start, end in matches:
                    span = doc[start:end]
                    phone_number_count+=1
                    write_stats_file.write("PHONE-NUMBER" + "---" + span.text.strip() + "\n")
...
def redact_date(file_content,write_stats_file):
    ...
    for l in file_content:
        ...
        for token in doc:
            ...
            if token.ent_type_ == 'DATE' or len(all)>0 or len(all2)>0 or len(all3)>0 or len(all4)>0:
                date_count+=1
...
def redact_gender(file_content,write_stats_file):
    ...
    '''
    gender_list = ["she","he","him","her","his","father","mother","man","woman","men","women","male","female","herself","himself"]
    op = []
    for l in file_content:
        doc = nlp_obj(l)
        '''
        ...
        for token in doc:
            #print(token,token.ent_type_)
            if token.ent_type_ == 'PERSON' or token.ent_type_ == 'GPE' or token.text.lower() in gender_list:
                if token.text.lower() in gender_list:
                    gender_count+=1
                    write_stats_file.write("GENDER" + "---" + token.text.lower() + "\n")
                for i in range(len(token)):
                    op.append('\u2588')
                op.append(" ")
            else:
